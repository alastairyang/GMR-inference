...
def compute_importance_sampling(
    ...
    for b in range(batch_num):
        ...
        Eb_sample = proposal.rvs(size=sample_size_per)
...
def compute_IS_stats(data_paths):
    """  
    Compute the expected value and variance of posterior
    using the saved weights and samples from importance sampling batches.

    data are large, so we do first pass to compute the expected value
    by sequentially summing and finally divide
    we do second pass to do similarly for variance
    """
    ...
    for idx, path in enumerate(data_paths):
        data       = np.load(path)
        Eb_sample  = data['Eb_sample']          # (65536, 500)
        weights    = data['weights']             # (500,)
        log_q      = data['log_q']               # (500,)

        weight_new = weights - log_q             # (500,)  — log-space correction
        weight_new = weight_new[np.newaxis, :]   # (1, 500) for broadcasting

        # Weighted sum: sum over the 500 samples axis
        batch_contribution = (Eb_sample * weight_new).sum(axis=1)  # (65536,)

        if weighted_sum is None:
            weighted_sum = np.zeros(Eb_sample.shape[0])             # (65536,)

        weighted_sum += batch_contribution                           # (65536,)
        total_weight += weight_new.sum()

        if idx % 10 == 0:
            print(f"Mean pass: processed {idx+1}/{len(data_paths)} batches...")

    weighted_mean = weighted_sum / total_weight  # (65536,)
    ...
    for idx, path in enumerate(data_paths):
        data = np.load(path)
        Eb_sample = data['Eb_sample']
        weights   = data['weights']
        log_q      = data['log_q']

        weight_new = weights - log_q
        weight_new = weight_new[np.newaxis, :]

        if weighted_var_sum is None:
            weighted_var_sum = np.zeros_like(Eb_sample[0])
        weighted_var_sum += np.sum(weight_new[:, np.newaxis] * (Eb_sample - weighted_mean) ** 2, axis=0)
        if idx % 10 == 0:
...
def _timeout_handler(signum, frame):
    raise TimeoutException("np.load timed out")

def load_with_retry(path, timeout_sec=30, max_retries=5, backoff=3.0):
    """
    Load a .npz file with a per-attempt timeout and exponential backoff retry.
    """
    signal.signal(signal.SIGALRM, _timeout_handler)

    for attempt in range(1, max_retries + 1):
        try:
            signal.alarm(timeout_sec)       # set countdown
            data = np.load(path)
            # Extract arrays while file handle is open & alarm still set
            result = {k: data[k] for k in data.files}
            signal.alarm(0)                 # cancel alarm on success
            return result
        except TimeoutException:
            print(f"  ⚠️  Timeout on attempt {attempt}/{max_retries} for {path}")
        except Exception as e:
            signal.alarm(0)
            raise e                         # re-raise non-timeout errors immediately

        wait = backoff * attempt
        print(f"  ↻  Retrying in {wait:.0f}s...")
        time.sleep(wait)

    raise RuntimeError(f"Failed to load {path} after {max_retries} attempts.")